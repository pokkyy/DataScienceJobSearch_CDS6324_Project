...
d3.csv("data/ds_salaries.csv", d3.autoType).then(function (data) {
    ...
    document.getElementById('filterButton').addEventListener('click', function() {
        ...
        state.selectedSalary = +document.getElementById('salaryUserInput').value;

        // Filter data based on user selections
        let filteredData = dataWrangled.data.filter(function(d) {
            return (!state.selectedEmploymentType || d.employment_type === selectedEmploymentType) &&
                   (!state.selectedExperienceLevel || d.experience_level === selectedExperienceLevel) &&
                   (!state.selectedJobTitle || d.job_title === selectedJobTitle) &&
                   (!state.selectedCompanySize || d.company_size === selectedCompanySize) &&
                   (!state.selectedSalary || +d.salary_in_usd >= selectedSalary);
        });
